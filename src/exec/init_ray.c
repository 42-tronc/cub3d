...
/*   Updated: 2023/10/24 13:34:50 by lboulatr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void get_ray_step_x(t_data *data, t_ray *ray, float dir_x)
{
	...
	else
	{
		ray->step.x = 1;
		ray->cardinal = EAST;
		ray->dist_to_wall.x = ((int)player_pos + 1.0f - player_pos) * ray->delta.x;
	}
...
static void get_ray_step_y(t_data *data, t_ray *ray, float dir_y)
{
	...
	else
	{
		ray->step.y = 1;
		ray->cardinal = NORTH;
		ray->dist_to_wall.y = ((int)player_pos + 1.0f - player_pos) * ray->delta.y;
	}
}
