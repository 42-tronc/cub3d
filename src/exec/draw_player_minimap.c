...
/*   Updated: 2023/10/30 10:47:37 by lboulatr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_player_minimap(t_mlx_data *minimap, float x, float y)
{
	...
	cut = SIZE_MAP / SIZE_CUT;
	p_position.x = (5 * SIZE_MAP);
	p_position.y = (5 * SIZE_MAP);
	while ((p_position.x) < (5 * SIZE_MAP) + cut)
	{
		p_position.y = 5 * SIZE_MAP;
		while ((p_position.y) < (5 * SIZE_MAP) + cut)
		{
			put_pixel(minimap, p_position.y + (SIZE_MAP / 3), p_position.x + (SIZE_MAP / 3), MAP_PLAYER);
			p_position.y++;
		}
		p_position.x++;
	}
